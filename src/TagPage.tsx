  ...
  return (
    <div className="flex flex-col gap-3 p-4 pb-20">
      <div className="flex items-center gap-2">
        <button className="text-[0.9rem] shrink-0" onClick={onBack}>← Back</button>
        ...
        <>
          ...
          >
            ...
            <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-[0.4rem] m-0 text-[0.9rem]">
              {tag.altTitle && <><dt className="text-[#888] text-xs uppercase tracking-[0.05em] self-center">Alt title</dt><dd className="m-0">{tag.altTitle}</dd></>}
              {tag.version && <><dt className="text-[#888] text-xs uppercase tracking-[0.05em] self-center">Version</dt><dd className="m-0">{tag.version}</dd></>}
              {tag.arranger && <><dt className="text-[#888] text-xs uppercase tracking-[0.05em] self-center">Arranger</dt><dd className="m-0">{tag.arranger}</dd></>}
              {tag.key && <><dt className="text-[#888] text-xs uppercase tracking-[0.05em] self-center">Key</dt><dd className="m-0">{tag.key}</dd></>}
              {tag.parts && <><dt className="text-[#888] text-xs uppercase tracking-[0.05em] self-center">Parts</dt><dd className="m-0">{tag.parts}</dd></>}
              {tag.type && <><dt className="text-[#888] text-xs uppercase tracking-[0.05em] self-center">Type</dt><dd className="m-0">{tag.type}</dd></>}
              {tag.ratingCount && tag.ratingCount !== '0' && (
                <><dt className="text-[#888] text-xs uppercase tracking-[0.05em] self-center">Rating</dt><dd className="m-0">{tag.rating} / 5 ({tag.ratingCount} ratings)</dd></>
              )}
            </dl>
          </div>
        </>
      ...
      {sheetUrl && error && (
        <div>
          <p className="text-[#f87171] m-0" role="alert">{error}</p>
