...
Once a file is loaded, the player SHALL display a waveform of the file's audio above the minimap scrubber, panning horizontally as playback position advances so the waveform content stays aligned with a playhead marker.
...
While the waveform is being computed for a newly loaded file, the player SHALL show a lightweight placeholder in its place, and playback, the minimap scrubber, and other transport controls SHALL remain usable during that computation.
...
- **THEN** the player shows a placeholder above the minimap scrubber, then replaces it with the file's waveform once computed

#### Scenario: Playback and transport remain usable while the waveform computes
- **WHEN** a user starts playback or uses the minimap scrubber while the waveform placeholder is still showing
- **THEN** playback and the minimap scrubber respond normally, unaffected by the waveform still being computed
